...
**** Workflow 4 installation and run on PUMA cluster
This workflow has an external dependency on library 'ripples', which can be built (sequential version) by running following script: 
#+BEGIN_SRC: bash
cd agile-workflows
...
Stage 1: Extract weighted coffee sale edges, by running the following:
#+BEGIN_SRC: bash
cd agile-workflows
./build/bin/do-workflow4 /lustre/scratch/feo/wk4/coffee.csv /lustre/scratch/feo/wk4/social.csv /lustre/scratch/feo/wk4/cyber.csv /lustre/scratch/feo/wk4/uses.csv /lustre/scratch/feo/wk4/commercial.csv coffee_sales_weights.csv
#+END_SRC

Stage 2: Run ripples code with the weighted sale edges as input. Note: Sequential version.
#+BEGIN_SRC: bash
cd agile-workflows
$HOME/ripples/build/release/tools/imm -i ./coffee_sales_weights.csv -w -p -k 100 -d IC -e 0.25 -o output.json
#+END_SRC

Stage 3: Re-run workflow 4 to reconfigure graph with the influential traders removed. Note: The influencers are manually removed in the current version of workflow 4.
#+BEGIN_SRC: bash
cd agile-workflows
./build/bin/do-workflow4 /lustre/scratch/feo/wk4/coffee.csv /lustre/scratch/feo/wk4/social.csv /lustre/scratch/feo/wk4/cyber.csv /lustre/scratch/feo/wk4/uses.csv /lustre/scratch/feo/wk4/commercial.csv coffee_sales_weights.csv
#+END_SRC

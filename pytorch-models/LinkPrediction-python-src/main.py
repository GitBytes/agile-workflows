...
from model_nodrnl import LinkPred
...
for data in train_loader:
    ...
    index = torch.zeros(data.batch.size(0),1,dtype=torch.long)

    # numer of nodes in each graph
    _, counts = torch.unique(data.batch, sorted=True, return_counts=True)
...
######  Code block below saves the model to a .pt file ######
num_features=30 #for AGILE, set num features to the right number (NUM_FEATURES)
...
torch.jit.save(model, 'LinkPred-nodrnl-noZ.pt')
###### end save model to file ######

###### Code block below loads model from a .pt file #####
model = torch.jit.load('LinkPred-nodrnl-noZ.pt')
###### end loading model from file
...
def train(loader,epoch):
    ...
    for data in tqdm(loader,desc="train"):  # Iterate in batches over the training dataset.
        data = data.to(device)
        label= data.label
        out = model(data.x, data.edge_index, data.edge_mask, data.batch)
        torch.cuda.empty_cache()

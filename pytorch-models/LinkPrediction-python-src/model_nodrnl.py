...
class LinkPred(MessagePassing):
    ...
    def __init__(self, in_channels: int, hidden_channels: int, heads: int = 1,\
        ...
        self.wp = WalkPooling(in_channels + hidden_channels*2,\
            hidden_channels, heads, walk_len).jittable()
...
class WalkPooling(MessagePassing):
    ...
    def attention_mlp(self, x: Tensor, edge_index: Tensor):
        ...
        row = edge_index[0]
        col = edge_index[1] 
        weights = (query[row] * key[col]).sum(dim=-1) / math.sqrt(self.hidden_channels)
        return weights

    def weight_encoder(self, x: Tensor, edge_index: Tensor, edge_mask: Tensor):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_mask: Tensor, batch: Tensor):
        ...
        link_ij = edge_index[:,torch.logical_not(edge_mask)][0]
        link_ji = edge_index[:,torch.logical_not(edge_mask)][1]
        ...
        for head in range(self.heads):
            ...
            # propagage once
            x_p = self.propagate(edge_index, x= x_p, norm = weights_p[:,head], size=None)
            x_m = self.propagate(edge_index, x= x_m, norm = weights_m[:,head], size=None)      

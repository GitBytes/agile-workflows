...
template <typename T> struct globalIdInserter {
...
static void exclusiveRecursiveScan(shad::rt::Handle &handle, uint64_t pos,
                                   VTYPE &elem, uint64_t &ndx, uint64_t &oid) {
  ...
  if (pos + nelems < size) {
    ...
    arrayPtr->AsyncApply(handle, pos + nelems, exclusiveRecursiveScan<VTYPE>,
                         my_ndx, oid);
  }

  for (uint64_t i = nelems - 1; i > 0; i--)
    (*data)[i].edges = (*data)[i - 1].edges + ndx;
  (*data)[0].edges = ndx;
}

template <typename VTYPE> void exclusiveScanVertices(uint64_t oid) {
  using arrayOID = shad::ObjectIdentifier<shad::Array<VTYPE>>;
  auto arrayPtr = shad::Array<VTYPE>::GetPtr((arrayOID)oid);

  auto localInclusiveScan = [](shad::rt::Handle &handle, const uint64_t &oid) {
    auto arrayPtr = shad::Array<VTYPE>::GetPtr((arrayOID)oid);
    std::vector<VTYPE> *data = arrayPtr->getData();

    uint64_t nelems = arrayPtr->getNElems();
    for (uint64_t i = 1; i < nelems; i++)
      (*data)[i].edges += (*data)[i - 1].edges;
  };
...
public:
  ...
  uint64_t
      edges; // GlobalIDS: number of edges ... Vertices: start index in Edges
  TYPES type;
...
public:
  ...
  Edge(uint64_t src_, uint64_t dst_, double weight_, TYPES type_,
       TYPES src_type_, TYPES dst_type_, uint64_t src_glbid_,
       uint64_t dst_glbid_) {
    src = src_;
    dst = dst_;
    weight = weight_;
...
using GlobalIDType = shad::Hashmap<uint64_t, Vertex, shad::MemCmp<uint64_t>,
                                   globalIdInserter<Vertex>>;

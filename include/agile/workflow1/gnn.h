...
template <typename TrainingState> void vcTrainLoop(TrainingState &TS) {
  ...
  InplaceFunctor F{TS.ReducerLocalPtrOID};
  auto localPtrs = shad::Array<uint64_t>::GetPtr(TS.ReducerLocalPtrOID);
  for (const auto &param : TS.Module.named_parameters()) {
    localPtrs->InsertAt(static_cast<uint32_t>(shad::rt::thisLocality()),
                        reinterpret_cast<uint64_t>(param.value.mutable_grad().data_ptr()));

    auto reducer = shad::MPIReducer<InplaceFunctor, InplaceFunctor>::Create(
      torchToMPITypes.at(param.value.mutable_grad().scalar_type()), F, F);

    reducer->AllReduce(MPI_SUM, param.value.mutable_grad().numel());
